...
function LogoPaymentFailContent() {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-gradient-to-b from-violet-50 to-[#fafaf8]">
      <div className="w-full max-w-sm flex flex-col gap-4">
        <div className="bg-white rounded-2xl shadow-sm border border-stone-100 p-8 flex flex-col items-center gap-4">
          ...
          <p className="font-bold text-stone-700 text-xl">{isCancel ? '결제 취소됨' : '결제 실패'}</p>
        ...
        <div className="flex gap-2">
          {orderId && (
            <button onClick={() => router.replace(`/admin/logo-payment?orderId=${orderId}`)}
...
export default function LogoPaymentFailPage() {
  return <Suspense><LogoPaymentFailContent /></Suspense>;
}
