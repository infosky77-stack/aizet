...
function LogoPaymentContent() {
  ...
  async function handlePay() {
    ...
    try {
      ...
      const base = {
        amount:    { currency: 'KRW' as const, value: PRICES.logo_generation },
        orderId,
        orderName: '로고/명함 자동 생성',
        successUrl: `${window.location.origin}/admin/logo-payment/success`,
        failUrl:    `${window.location.origin}/admin/logo-payment/fail`,
      };
      ...
      if (method === 'CARD') {
        ...
        await (payment.requestPayment as any)({ method: 'EASY_PAY', ...base, easyPay: { easyPayProvider: method } });
      }
    } catch (err: unknown) {
      ...
      <div className="w-full max-w-sm flex flex-col gap-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="font-bold text-violet-800 text-sm">로고 / 명함 자동 생성</p>
              <p className="text-xs text-stone-400">결제 후 AI가 로고 3가지를 만들어 드립니다</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-stone-100 p-5 flex flex-col gap-3">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-stone-500">로고 3종 생성 + 명함 합성</span>
            <span className="text-stone-800 font-medium">{PRICES.logo_generation.toLocaleString()}원</span>
          </div>
          <div className="flex justify-between font-bold border-t border-stone-100 pt-3">
            <span>총 결제금액</span>
            <span className="text-violet-700 text-lg">{PRICES.logo_generation.toLocaleString()}원</span>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-stone-100 p-5 flex flex-col gap-4">
          ...
          <button onClick={handlePay} disabled={!method || loading}
            ...
            {loading
              ...
              : `${PRICES.logo_generation.toLocaleString()}원 결제하기`}
          </button>

          {method && <p className="text-xs text-stone-400 text-center">토스페이먼츠 안전결제로 연결됩니다</p>}
        </div>
...
export default function LogoPaymentPage() {
  return (
    <Suspense>
      <LogoPaymentContent />
    </Suspense>
