...
// 구매자 장바구니 + 주문서 — v1은 주문 접수까지(무통장/연락 후 결제 안내).
// 항목은 slug 스코프 스토어에서, 최종 가격 검증은 서버(주문 API)가 담당한다.
    ...
    <div className="min-h-screen bg-stone-50">
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur border-b border-stone-100">
        <div className="max-w-3xl mx-auto px-4 h-14 flex items-center gap-3">
          <Link href={`/site/${slug}/shop`} className="p-1.5 rounded-lg hover:bg-stone-100 text-stone-500">
            <ArrowLeft size={18} />
          </Link>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 flex flex-col gap-6">
        {items.length === 0 ? (
          <div className="flex flex-col items-center gap-3 py-20 text-stone-400">
            ...
            <Link href={`/site/${slug}/shop`} className="text-sm font-semibold text-red-500 hover:underline">
              상품 보러 가기
            </Link>
          </div>
        ) : (
