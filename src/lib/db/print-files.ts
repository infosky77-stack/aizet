...
export function createClient(data: Omit<Client, 'id' | 'createdAt'>): Client {
  const client: Client = { ...data, id: `c-${String(clients.length + 1).padStart(3, '0')}`, createdAt: new Date().toISOString() };
...
export function addClientFile(data: Omit<ClientFile, 'id' | 'uploadedAt'>): ClientFile {
  ...
  const file: ClientFile = { ...data, id: `f-${String(files.length + 1).padStart(3, '0')}`, uploadedAt: new Date().toISOString() };
  files = [...files, file];
  return file;
}
...
export function saveLabel(data: Omit<GeneratedLabel, 'id' | 'createdAt'>): GeneratedLabel {
  const label: GeneratedLabel = { ...data, id: `lbl-${String(_labelIdSeq++).padStart(3, '0')}`, createdAt: new Date().toISOString() };
  labels = [...labels, label];
  return label;
}
