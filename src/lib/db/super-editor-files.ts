...
export function insertFile(params: InsertFileParams): SuperEditorFile {
  const id  = randomUUID();
  const now = Date.now();
  db.prepare(`
...
export function listFiles(userId: string, fileType?: FileType, orderId?: string): SuperEditorFile[] {
...
export function findExactDuplicate(
  ...
  return db.prepare<[string, string, string, string | null], SuperEditorFile>(
...
export function listOrigNames(userId: string, orderId: string | null = null, excludeId?: string): Set<string> {
  const rows = excludeId
    ? db.prepare<[string, string | null, string], { orig_name: string }>(
        'SELECT orig_name FROM super_editor_files WHERE user_id=? AND order_id IS ? AND id!=?'
      ).all(userId, orderId, excludeId)
    : db.prepare<[string, string | null], { orig_name: string }>(
...
export function renameFile(id: string, userId: string, newName: string): SuperEditorFile | null {
  const record = getFile(id);
  if (!record || record.user_id !== userId) return null;
  // 이 파일이 속한 주문(폴더) 안에서만 이름충돌 검사 — 다른 주문의 동명 파일과는 무관
  const existingNames = listOrigNames(userId, record.order_id, id);
  const finalName = resolveAvailableName(existingNames, newName);
  db.prepare('UPDATE super_editor_files SET orig_name=? WHERE id=? AND user_id=?').run(finalName, id, userId);
  return getFile(id);
}

// order: 원하는 순서대로 나열된 파일 id 배열 (0번째가 맨 앞)
export function reorderFiles(userId: string, order: string[]): void {
  const stmt = db.prepare('UPDATE super_editor_files SET sort_order=? WHERE id=? AND user_id=?');
