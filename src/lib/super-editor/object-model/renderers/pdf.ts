...
// 임베드한 NotoSansKR로만 그린다. 폰트 임베드는 반드시 subset:false(한글 글리프 누락 방지 —
// 기존 pdf/shared.ts 검증 규칙). 렌더러는 Object Model(블록)을 읽기만 하고, 레이아웃 상수
// (마진·폰트크기)는 이 파일 내부 상수로 둔다(블록엔 꾸밈이 없다). 모르는 kind는 건너뛴다.
//
